...
<div id="posts" class="span-16 prepend-1 append-1">
	...
	<div class="posttitle">
		<?php if (have_posts()) : ?>
		<?php $post = $posts[0]; ?>
		<?php if (is_category()) { ?><h2>Archives '<?php echo single_cat_title(); ?>'</h2>
		<?php } elseif (is_day()) { ?><h2>Archives <?php the_time('F jS, Y'); ?></h2>
		<?php } elseif (is_month()) { ?><h2>Archives <?php the_time('F, Y'); ?></h2>
		<?php } elseif (is_year()) { ?><h2>Yearly archives <?php the_time('Y'); ?></h2>
	...
	<div class="post">
		<h2>Not found!</h2>
		<p><?php _e('Oh. Ya see, we had here some p0rn, but it was erased. I am reeeally sorry, guy.'); ?></p>
