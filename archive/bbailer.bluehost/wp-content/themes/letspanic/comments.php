<?php // Do not delete these lines
	...
	if (!empty($post->post_password)) { // if there's a password
		if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
			...
			<p class="nocomments"><?php _e('Enter password to watch comments.'); ?></p>
...
<div class="allcomments">
	...
	<?php if ('open' == $post->comment_status) : ?>
		...
		<div id="commentform">
			...
			<p>
				<?php if ( $user_ID ) : ?>
					You logged in as <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="<?php _e('Log out of this account') ?>">Logout &raquo;</a><br/><br/>
				<?php else : ?>
					<label for="author">Name <?php if ($req) _e('<strong>обязательно</strong>'); ?></label><br/>
					<input type="text" name="author" id="name" class="text" value="<?php echo $comment_author; ?>" size="22" tabindex="1" /><br/>

					<label for="email">E-mail (never published) <?php if ($req) _e('<strong>обязательно</strong>'); ?></label><br/>
